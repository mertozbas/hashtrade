...
def balance(
    ...
    try:
        ...
        if action == "get":
            ...
            # Fetch balance using bybit_v5 tool directly
            import importlib.util
            spec = importlib.util.spec_from_file_location("bybit_v5", Path(__file__).parent / "bybit_v5.py")
            bybit_module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(bybit_module)
            
            # Call bybit_v5 to get balance
            result = bybit_module.bybit_v5(action="get_balance")
            ...
            if equity == 0:
                equity_match = re.search(r'(?:Toplam|Total)\s+(?:Değer|Equity):\s*\$?([\d,.]+)', balance_text, re.IGNORECASE)
                if equity_match:
            ...
            record = {
                ...
                "raw_data": balance_text[:500]  # First 500 chars to avoid huge JSON
            }
            ...
            for i, record in enumerate(balance_data["history"][-10:], 1):  # Last 10
