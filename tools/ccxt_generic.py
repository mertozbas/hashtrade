...
def ccxt_multi_exchange_orderbook(
    ...
    try:
        ...
        for r in results:
            if 'error' in r:
                result_text += f"[ERROR] {r['exchange']}: {r['error']}\n"
            else:
                result_text += f"[OK] {r['exchange']}: Best bid ${r['best_bid'][0]:.2f}, "
